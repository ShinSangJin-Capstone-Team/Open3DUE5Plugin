        ...
        if (errorCode == Synexens::SYERRORCODE_SUCCESS && nCount > 0)
        {
            ...
            if (errorCode == Synexens::SYERRORCODE_SUCCESS)
            {
                ...
                while (true)
                {
                    ...
                    case 's': // S Switch data stream type
                    {
                        for (int nDeviceIndex = 0; nDeviceIndex < nCount; nDeviceIndex++)
                        {
                            ...
                            case Synexens::SYDEVICETYPE_CS30_DUAL:
                            {
                                ...
                                if (itStreamFind != g_mapStreamType.end())
                                {
                                    ...
                                    case Synexens::SYSTREAMTYPE_RGBD:
                                    {
                                        ...
                                        Synexens::SYErrorCode errorCodeSwitch = Synexens::SetFrameResolution(pDeviceInfo[nDeviceIndex].m_nDeviceID, Synexens::SYFRAMETYPE_RAW, Synexens::SYRESOLUTION_640_480);
                                        if (errorCodeSwitch != Synexens::SYERRORCODE_SUCCESS)
                                        {
                                            bSuccess = false;
                                            PrintErrorCode("SetFrameResolution RAW", errorCodeSwitch);
                                        }
                                        break;
                                    }
                                    default:
                                    {
                                        ...
                                        if (itStreamFind->second == Synexens::SYSTREAMTYPE_RGBD)
                                        {
                                            Synexens::SYErrorCode errorCodeSwitch = Synexens::SetFrameResolution(pDeviceInfo[nDeviceIndex].m_nDeviceID, Synexens::SYFRAMETYPE_DEPTH, Synexens::SYRESOLUTION_640_480);
                                            if (errorCode == Synexens::SYERRORCODE_SUCCESS)
                                            {
                                                errorCodeSwitch = Synexens::SetFrameResolution(pDeviceInfo[nDeviceIndex].m_nDeviceID, Synexens::SYFRAMETYPE_RGB, Synexens::SYRESOLUTION_1920_1080);
                                                if (errorCodeSwitch != Synexens::SYERRORCODE_SUCCESS)
                                                {
                                                    bSuccess = false;
                                                    PrintErrorCode("SetFrameResolution RGB", errorCodeSwitch);
                                                }
                                            }
                                            else
                                            {
                            ...
                            case Synexens::SYDEVICETYPE_CS20_P:
                            {
                                ...
                                if (itStreamFind != g_mapStreamType.end())
                                {
                                    ...
                                    {
                                        itStreamFind->second = Synexens::SYSTREAMTYPE_RAW;
                                        Synexens::SYErrorCode errorCodeSwitch = Synexens::SetFrameResolution(pDeviceInfo[nDeviceIndex].m_nDeviceID, Synexens::SYFRAMETYPE_RAW, Synexens::SYRESOLUTION_320_240);
                                        if (errorCodeSwitch != Synexens::SYERRORCODE_SUCCESS)
                                        {
                                            bSuccess = false;
                                            PrintErrorCode("SetFrameResolution RAW", errorCodeSwitch);
                                    ...
                                    {
                                        itStreamFind->second = Synexens::SYSTREAMTYPE_DEPTH;
                                        Synexens::SYErrorCode errorCodeSwitch = Synexens::SetFrameResolution(pDeviceInfo[nDeviceIndex].m_nDeviceID, Synexens::SYFRAMETYPE_DEPTH, Synexens::SYRESOLUTION_320_240);
                                        if (errorCodeSwitch != Synexens::SYERRORCODE_SUCCESS)
                                        {
                                            bSuccess = false;
                                            PrintErrorCode("SetFrameResolution RAW", errorCodeSwitch);
                    ...
                    case 'l': // L Switch to low resolution
                    {
                        for (int nDeviceIndex = 0; nDeviceIndex < nCount; nDeviceIndex++)
                        {
                            switch (pDeviceInfo[nDeviceIndex].m_deviceType)
                    ...
                    case 'h': // H Switch to high resolution
                    {
                        for (int nDeviceIndex = 0; nDeviceIndex < nCount; nDeviceIndex++)
                        {
                            switch (pDeviceInfo[nDeviceIndex].m_deviceType)
                    ...
                    case 'u': // U Increase credit time
                    {
                        for (int nDeviceIndex = 0; nDeviceIndex < nCount; nDeviceIndex++)
                        {
                            ...
                            else
                            {
                                ...
                                errorCodeIntegralTime = Synexens::SetIntegralTime(pDeviceInfo[nDeviceIndex].m_nDeviceID, pIntegralTime[nDeviceIndex]);
                                if (errorCodeIntegralTime == Synexens::SYERRORCODE_SUCCESS)
                                {
                    ...
                    case 'm': // M Mirror - Horizontal mirroring
                    {
                        for (int nDeviceIndex = 0; nDeviceIndex < nCount; nDeviceIndex++)
                        {
                            bool bMirror = false;
                    ...
                    case 'p': // p Save point cloud data
                    {
                        for (int nDeviceIndex = 0; nDeviceIndex < nCount; nDeviceIndex++)
                        {
